...
class FileKVDB {
  private:
    ...
    string joinPathWithPrefix( int level ) { assert( level>=0 && level<4 );return joinPath(level)+dbPrefix[level];};
    string DBPath( int level ){ assert( level>=0 && level<4 );return realName( dbPath[level], level );};
    void DBPath( int level, string mypath ){ assert( level>=0 && level<4 );dbPath[level]=dbPrefix[level]+mypath;};
    string realName ( string str, int level ){ assert( level>=0 && level<4 );int pos=str.find('_');if(pos!=str.npos) return str.substr(pos+1); else return str;};
